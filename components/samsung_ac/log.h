...
#define LOG_RAW(dir, inter, ...) ({ if (debug_log_raw_bytes) LOGD("%s +%d: %s", dir, inter, bytes_to_hex(__VA_ARGS__).c_str()); })
#define LOG_RAW_DISCARDED(dir, inter, ...) ({if (debug_log_raw_bytes) LOGV("%s +%d: %s", dir, inter, bytes_to_hex(__VA_ARGS__).c_str()); })
